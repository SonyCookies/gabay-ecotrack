...
import { 
  User, Mail, Phone, Lock, Save, ShieldCheck, 
  Activity, CheckCircle2, Circle, Loader2, ChevronRight,
...
export default function AdminProfilePage() {
  ...
  // Fetch latest data once authenticated
  useEffect(() => {
    ...
    fetchUserData();
  }, [dispatch, reduxAuth.isAuthenticated]);
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 pt-8 space-y-6 mx-auto">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
             <div className="flex items-center gap-6 bg-white/10 backdrop-blur-md px-8 py-3 rounded-2xl border border-white/10 w-full lg:max-w-xl shadow-2xl shadow-brand-900/40 translate-y-1">
                <div className="w-14 h-14 rounded-2xl bg-brand-600 flex items-center justify-center text-white font-bold text-2xl shadow-xl shadow-brand-600/20">
                  {formData.displayName.charAt(0)}
                </div>
                <div>
                  <p className="text-xl font-black text-white tracking-tight leading-none">{formData.displayName}</p>
                  <p className="text-xs font-bold text-brand-200/60 lowercase tracking-tight mt-1.5">{formData.email}</p>
                </div>
             </div>
          </div>
        </div>
                      ...
                      <button
                          ...
                          className={`group relative flex items-center p-4 rounded-2xl border transition-all text-left ${
                              activeTab === item.id 
                                  ...
                                  : "bg-white/80 backdrop-blur-sm border-gray-100/50 hover:border-gray-200 hover:shadow-sm"
                          }`}
                      >
                          ...
                          <div className="flex-1">
                              ...
                              <p className={`text-[10px] font-bold tracking-tight mt-0.5 ${activeTab === item.id ? "text-brand-600" : "text-gray-400"}`}>
                                  {item.sub}
                              </p>
                          </div>
          ...
          <div className="lg:col-span-8">
            <div className="bg-white rounded-3xl border border-gray-100 shadow-2xl shadow-brand-900/10 overflow-hidden flex flex-col min-h-[500px]">
              ...
              <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                ...
                <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center text-brand-600 border border-gray-100 shadow-sm">
              ...
              <div className="flex-1 p-8">
                {activeTab === "personal" && (
                  <form onSubmit={handleSaveProfile} className="space-y-8">
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-semibold text-gray-700 ml-0.5 mb-2">Title / Department</label>
                        <div className="relative group">
                          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 group-focus-within:text-brand-500 transition-colors">
                  ...
                  <form onSubmit={handleUpdatePassword} className="space-y-8">
                      <div className="space-y-6 max-w-xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="block text-sm font-semibold text-gray-700 ml-0.5 mb-2">New Secure Password</label>
                                <div className="relative group">
                                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 group-focus-within:text-brand-500 transition-colors">
                                    <Lock className="h-5 w-5" />
                                  </div>
                        ...
                        <div className="mt-8 space-y-4 px-1">
                            <div className="flex justify-between items-end mb-1">
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-widest ${requirementsMet === 5 ? 'text-brand-600' : 'text-brand-500'}`}>
                                    {requirementsMet === 0 ? 'Weak' : requirementsMet <= 2 ? 'Fair' : requirementsMet <= 4 ? 'Good' : 'Strong'}
                                </span>
                            </div>
                            <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden flex gap-0.5 p-0.5 shadow-inner">
                                <div 
                                    className="h-full bg-gradient-to-r from-brand-600 to-brand-400 rounded-full transition-all duration-700 shadow-[0_0_12px_rgba(37,99,235,0.4)]"
                                    style={{ width: `${strengthPercent}%` }}
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-y-3 gap-x-6 pt-3">
                                {requirements.map((req, idx) => (
                                    <div key={idx} className="flex items-center gap-2.5 group/req">
                                        <div className="transition-all duration-300">
                                            {req.met ? (
                                              <CheckCircle2 className="w-4 h-4 text-brand-500 drop-shadow-[0_0_2px_rgba(37,99,235,0.2)]" />
                                            ) : (
                                              <Circle className="w-4 h-4 text-gray-200 group-hover/req:text-gray-300 transition-colors" />
                                            )}
                                        </div>
                                        <span className={`text-[11px] font-bold transition-colors ${req.met ? 'text-gray-900' : 'text-gray-400'}`}>
                                            {req.label}
                                        </span>
                                    </div>
                                ))}
